...
{
?>
    <div class="column_list_item column_class_<?= $table ?>__<?= $column ?>">
        <div class="float">
            <input type="checkbox" id="columnsort_<?= $table ?>__<?= $column ?>" onclick="toggle_columnsort('<?= $table ?>', '<?= $column ?>')"/> <span id="selected_column_<?= $table ?>__<?= $column ?>"><?= $column ?></span>
        </div>
